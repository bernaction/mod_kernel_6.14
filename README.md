...
Este guia apresenta o passo a passo completo para compilar e instalar um kernel Linux com patch PREEMPT_RT, transformando seu Lubuntu em um sistema operacional de tempo real.
...
- Lubuntu instalado (versão 22.04 ou superior recomendada)
...
```bash
wget https://cdn.kernel.org/pub/linux/kernel/projects/rt/6.14/patch-6.14-rt7.patch.xz
```
...
```bash
xzcat ../patch-6.14-rt7.patch.xz | patch -p1
```
**O que faz:**
...
sudo update-grub
```
**O que faz:** Aplica as alterações feitas no arquivo de configuração do GRUB.
...
**Autor:** Roteiro para modificação de kernel Lubuntu para RTOS  
**Versão:** 1.0  
**Data:** 2024
